...
var _Waypoint = class extends import_obsidian.Plugin {
	constructor() {
				...
				for (let i = 0; i < lines.length; i++) {
					if (lines[i].trim() === this.settings.waypointFlag) {
						if (this.isFolderNote(file)) {
							...
							yield this.updateParentWaypoint(
								file.parent,
		...
		this.updateParentWaypoint = (node, includeCurrentNode) =>
			__async(this, null, function* () {
	...
	isFolderNote(file) {
		if (this.settings.folderNoteType === FolderNoteType.InsideFolder) {
			...
			if (file.parent) {
				return (
					this.app.vault.getAbstractFileByPath(this.getCleanParentPath(file) + file.basename) instanceof
					import_obsidian.TFolder
				);
			}
			return false;
		}
					...
					if (!this.settings.showFolderNotes) {
						if (this.settings.folderNoteType === FolderNoteType.InsideFolder) {
							...
							children = children.filter(
								(child) => child instanceof import_obsidian.TFolder || !folderNames.has(child.name)
							);
						}
					}
...
var WaypointSettingsTab = class extends import_obsidian.PluginSettingTab {
	...
	display() {
		...
		new import_obsidian.Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle.setValue(this.plugin.settings.showFolderNotes).onChange((value) =>
					__async(this, null, function* () {
		...
		new import_obsidian.Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle.setValue(this.plugin.settings.showNonMarkdownFiles).onChange((value) =>
					__async(this, null, function* () {
		...
		new import_obsidian.Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle.setValue(this.plugin.settings.showEnclosingNote).onChange((value) =>
					__async(this, null, function* () {
		...
		new import_obsidian.Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle.setValue(this.plugin.settings.stopScanAtFolderNotes).onChange((value) =>
					__async(this, null, function* () {
		...
		new import_obsidian.Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle.setValue(this.plugin.settings.useWikiLinks).onChange((value) =>
					__async(this, null, function* () {
